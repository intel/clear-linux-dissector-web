...
class Update(models.Model):
    ...
    def get_files_dir(self):
        import settings
        udldir = getattr(settings, 'UPDATE_FILES_DIR', '')
        if udldir:
            return os.path.join(udldir, str(self.id))
        return None
...
class UpdateFile(models.Model):
    ...
    def get_filepath(self):
        updatedir = self.update.get_files_dir()
        if updatedir:
            return os.path.join(updatedir, self.filename)
        return None
    ...
    def __str__(self):
        return '%s: %s%s' % (self.update, self.filename, ' (exists)' if self.exists() else '')

@receiver(models.signals.post_delete, sender=UpdateFile)
def delete_update_file(sender, instance, *args, **kwargs):
    # Ensure that files generated by an update get deleted when it is deleted
